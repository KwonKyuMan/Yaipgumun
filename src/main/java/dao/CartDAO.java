...
public class CartDAO {
    ...
    //목록 배열 
    public ArrayList<Cart> getAllMember(int startRow, int endRow, String id) {
        d.getCon();

        ArrayList<Cart> v = new ArrayList<Cart>();

        try {
            String sql = "select c.*, i.* from cart c inner join item i on c.it_uid = i.it_uid where c.mb_id=? and c.ct_status='준비'";
            sql += " ORDER BY c.ct_uid DESC LIMIT ?, ?";
