...
public class UserOrderDAO {
    ...
    //주문목록
  	public ArrayList<Orders> getOrderList(String id, int startRow, int endRow, String field, String search) {
  		...
  		try {
  			String sql = "SELECT o.or_date, o.od_id, o.ct_status, o.mb_id, o.name, SUM(o.it_price) AS total_price, i.file1_thumb FROM orders o INNER JOIN item i ON o.it_uid = i.it_uid WHERE o.mb_id = ? GROUP BY o.od_id ORDER BY o.or_date DESC LIMIT ?, ?";
  			
  			if (search != null && !search.equals("")) {
  			    sql = "SELECT o.or_date, o.od_id, o.ct_status, o.mb_id, o.name, SUM(o.it_price) AS total_price, i.file1_thumb FROM orders o INNER JOIN item i ON o.it_uid = i.it_uid WHERE o.mb_id = ? AND " + field + " LIKE '%" + search + "%' GROUP BY o.od_id ORDER BY o.or_date DESC LIMIT ?, ?";
  			}
    ...
    //주문 총개수
  	public int getAllcount(String id, String field, String search) {
  		...
  		int count = 0;
  		try {
  			String sql = "SELECT COUNT(*) AS group_count FROM (SELECT COUNT(*) FROM orders WHERE mb_id = ? AND ct_status <> '준비' GROUP BY od_id) AS grouped_orders";
